...
export const ShortUrlTest = () => { 
        ...
        const testData = {
            existingRecord : { 
                ...
                password: 'ef7efc9839c3ee036f023e9635bc3b056d6ee2db',
                token: 'ef7efc9839c3ee036f023e9635bc3b056d6ee2db'
            },
            nonExistingRecord : { 
                uid: '11111111',
        ...
        describe('Testing ShortUrl.Service', () => { 
                test('Get ShortUrl By UId: Existing url', async () => { 
                    const result =  await AuthService.login( testData.existingRecord.email,  testData.existingRecord.password,  testData.existingRecord.token );
                    expect(result.id).toBe( testData.existingRecord.id);
                    expect(result.email).toBe( testData.existingRecord.email);
                });
        ...
        describe('Testing ShortUrl API', () => { 
                ...
                //     .get('/auth/login/'+ testData.existingRecord.id)
                //     .set('Accept', 'application/json');  
                    
                //     expect(response.headers["content-type"]).toMatch(/json/);
                //     expect(response.headers["content-length"]-1).toBeGreaterThan(10);
                //     expect(response.status).toEqual(200); 
                //     expect(response.body.id).toBe( testData.existingRecord.id); 
                //     expect(response.body.email).toBe( testData.existingRecord.email); 
                // });
